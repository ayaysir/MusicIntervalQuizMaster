...
public class CustomAlertView: UIView, AlertViewProtocol {
  ...
  private var iconTintColor: UIColor = .green
  ...
  open func present(on view: UIView, completion: (()->Void)? = nil) {
    ...
    alpha = 0
    sizeToFit()
    // 중앙보다 위로 이동하도록 y값을 조정
    center = .init(x: viewForPresent.frame.midX, y: viewForPresent.frame.midY - 150)
